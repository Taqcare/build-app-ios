...
export const scheduleNextSuggestedSessionReminder = (lastCompletedDate: Date) => {
  // Calcular próxima data sugerida (assumindo intervalo de 7 dias)
  const nextSuggestedDate = new Date(lastCompletedDate);
  nextSuggestedDate.setDate(nextSuggestedDate.getDate() + 7);
  ...
  if (nextSuggestedDate > today) {
    // Agendar lembretes para 2 e 1 dia antes da data sugerida
    [2, 1].forEach((daysBefore, index) => {
      ...
      if (reminderDate > new Date()) {
        ...
        scheduleSessionNotification({
          id: 300000 + index, // IDs únicos para lembretes de sugestão
          date: reminderDate,
          title: 'Hora da Próxima Sessão!',
          body: `${daysText} é um bom dia para sua próxima sessão de tratamento IPL. Que tal agendar?`
        });
      }
    });
  ...
  if (scheduledSessions.length > 0) {
    ...
    const scheduleTimedNotifications = () => {
      const notificationTimes = [
        new Date(today.setHours(9, 0, 0, 0)), // Morning notification
        new Date(today.setHours(12, 0, 0, 0)), // Noon notification  
        new Date(today.setHours(16, 0, 0, 0)), // Afternoon notification
      ];
      
      notificationTimes.forEach((time, index) => {
        if (time > new Date()) { // Only schedule notifications that are in the future
          scheduleSessionNotification({
            id: 100000 + index, // Unique ID for each notification
            date: time,
